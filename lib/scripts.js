...
function callScript(client, name, args) {
  debuglog(name, ...args);
...
const scripts = {
  isJobInList(client, listKey, jobId) {
    return callScript(client, 'isJobInList', [listKey, jobId]).then(result => {
      return result === 1;
    });
  },
  ...
  addJob(client, queue, job, opts) {
    ...
    return callScript(client, 'addJob', keys);
  },
  ...
  moveToActive(queue, jobId) {
    ...
    return callScript(queue.client, 'moveToActive', keys.concat(args)).then(
      raw2jobData
    );
  },
  ...
  moveToFinished(job, val, propVal, shouldRemove, target, ignoreLock) {
    ...
    return callScript(job.queue.client, 'moveToFinished', args).then(result => {
      if (result < 0) {
  ...
  isFinished(job) {
    ...
    return callScript(job.queue.client, 'isFinished', keys.concat([job.id]));
  },
  ...
  moveToDelayed(queue, jobId, timestamp, ignoreLock) {
    ...
    return callScript(queue.client, 'moveToDelayed', args).then(result => {
      switch (result) {
        case -1:
          throw new Error(
            'Missing Job ' +
  ...
  extendLock(queue, jobId) {
    ...
    return callScript(queue.client, 'extendLock', args);
  },

  releaseLock(queue, jobId) {
    const args = [queue.toKey(jobId) + ':lock', queue.token];
    return callScript(queue.client, 'releaseLock', args);
  },

  takeLock(queue, job) {
    const args = [job.lockKey(), queue.token, queue.settings.lockDuration];

    return callScript(queue.client, 'takeLock', args);
  },
  ...
  */
  updateDelaySet(queue, delayedTimestamp) {
    ...
    return callScript(queue.client, 'updateDelaySet', keys.concat(args));
  },
  ...
  promote(queue, jobId) {
    ...
    return callScript(queue.client, 'promote', keys.concat(args));
  },
  ...
  moveUnlockedJobsToWait(queue) {
    ...
    return callScript(queue.client, 'moveStalledJobsToWait', keys.concat(args));
  },
  ...
  reprocessJob(job, options) {
    ...
    return callScript(queue.client, 'reprocessJob', keys.concat(args));
  }
};
